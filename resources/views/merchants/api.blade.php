...
@section('content')
    ...
    <div class="content-header">
        ...
        <div class="row mb-2">
            <div class="col-sm-9">
                <h1 class="m-0">Merchant API</h1>
            </div><!-- /.col -->
            <div class="col-sm-3">
                <div class="btn-toolbar float-right">
                    @if(in_array(11,$permitions))
                        <a href="{{switchUrl('download-postman-collection')}}" target="_blank" class="btn btn-success mr-2"><i class="fa fa-download"></i> DOWNLOAD KEYS JSON POSTMAN</a>
                    @endif
                </div>
            </div><!-- /.col -->
        </div><!-- /.row -->
                                ...
                                <div class="col-md-12" style="margin-top:15px" >
                                    <div class="card">
                                        ...
                                        <div class="col-lg-6 col-xs-12">
                                            <h5>URL {{$titleEnvironments}}</h5>
                                            <div class="input-group margin15">
                                            <span class="input-group-text" id="basic-addon1">baseUrl</span>
                                            <input type="text" class="form-control" value="{{$urlBase}}">
                                            </div>
                                            <br/>
                                        </div>
                                        @if(in_array(9,$permitions))
                                            <div class="col-lg-6 col-xs-12" >
                                            <h5>HEADER DEPOSIT</h5>
                                            <div class="input-group margin15">
                                                <span class="input-group-text" id="basic-addon1">Token</span>
                                                <input type="text" name="token" id="token" value="{{auth()->user()->client->key->authorization}}" class="form-control">
                                            </div>
                                            </div>
                                            <br/>
                                        @endif

                                        @if(in_array(10,$permitions))

                                            @if(auth()->user()->client->key->authorization == '')
                                                <div class="col-md-12 col-xs-12" >
                                                    <form action="{{switchUrl('merchants/update_api_keys/'.auth()->user()->client->id)}}" method="POST">
                                                    {{ csrf_field() }}
                                                    <input type="hidden" name="action" value="create">
                                                    <button type="submit" class="btn btn-success btn-block" style="width: 100%;"><i class="fa fa-key"></i> CREATE TOKEN</button>
                                                    </form>
                                                </div>
                                                <br/>
                                            @else
                                                <div class="col-md-12 col-xs-12" >
                                                    <a href="{{switchUrl('merchants/update_api_keys/'.auth()->user()->client->id)}}" class="confirmation-api-keys"><button type="button" class="btn btn-primary" style="width:100%;"><i class="fa fa-key"></i> UPDATE TOKEN</button></a>
                                                </div>
                                                <br/>
                                            @endif
                                ...
                                <div class="col-md-12" style="margin-top:15px">
                                    <div class="card">
                                        ...
                                        @if(auth()->user()->level == 'merchant')
                                            @if(auth()->user()->client->key->authorization != '')
                                            <form action="{{switchUrl('/merchants/update_webhook')}}" method="POST">
                                                <div class="row">
                                                    {{ csrf_field() }}
                                                    ...
                                                    <div class="col-lg-6 col-xs-12" >
                                                        ...
                                                        <label for="">URL Callback Deposit</label>
                                                        <input type="text" name="url_callback" id="url_callback" value="{{auth()->user()->client->key->url_callback}}" class="form-control">
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-lg-6 col-xs-12" >
                                                        ...
                                                        <input type="text" name="url_callback_withdraw" id="url_callback_withdraw" value="{{auth()->user()->client->key->url_callback_withdraw}}" class="form-control">
                                                        </div>
                                                    </div>
