...
class CheckHookPIXMetaPay implements ShouldQueue
{
        ...
        if($transaction){
            ...
            if($payment_id == $transaction->payment_id){
                ...
                try{
                    ...
                    $path_name = "nexapay-webhook-send-".date("Y-m-d");

                    if (!file_exists('/var/www/html/nexapay/logs/'.$path_name)) {
                        mkdir('/var/www/html/nexapay/logs/'.$path_name, 0777, true);
                    }

                    $FunctionsController->registerRecivedsRequests("/var/www/html/nexapay/logs/".$path_name."/metapay.txt",json_encode($post_register));
