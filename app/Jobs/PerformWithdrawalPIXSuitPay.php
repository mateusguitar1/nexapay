...
class PerformWithdrawalPIXSuitPay implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        switch($type_pix){
            case"phone":
                ...
                $type_pixkey = "phoneNumber";
            break;
            case"cpf":
                $new_pixkey = str_replace(".","",$pix_key);
                $new_pixkey = str_replace("-","",$new_pixkey);
                $type_pixkey = "document";
            break;
            case"cnpj":
                $new_pixkey = str_replace(".","",$pix_key);
                $new_pixkey = str_replace("-","",$new_pixkey);
		...
		$postData = [
            ...
            "key" => $new_pixkey,
            "typeKey" => $type_pixkey,
            "callbackUrl" => "https://hooknexapay.financebaking.com/api/suitpayhookwithdraw",
            "documentValidation" => $user_document,
            "externalId" => $transaction->order_id
		];

        $path_name = "suitpay-withdraw-data-send-".date("Y-m-d");
        ...
        $path_name = "suitpay-response-raw-cashout-".date("Y-m-d");
		...
		if (curl_errno($curl)) {
            ...
            $path_name = "suitpay-save-error-".date("Y-m-d");
            ...
            $path_name = "result-suitpay-cashout-".date("Y-m-d");
            ...
            if(isset($get_response['idTransaction'])){
                ...
                if($get_response['response'] == "OK"){
                    ...
                    try{
                        ...
                        //Do update
                        $transaction->update([
                            "payment_id" => $get_response['idTransaction']
                        ]);
