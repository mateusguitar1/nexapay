...
class PerformWithdrawalPIXLuxTakNewsts implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $url = "https://api.luxtak.com/api/payout";
        ...
        $user_document = "45403734000150";

		$postData = [
            "name" => $user_name,
            "phone" => "84938297221",
            "email" => "client@fastpayments.com.br",
            "amount" => floatval($amount),
            "arrival_currency" => "BRL", // fixed value => BRL
            "custom_code"  => $transaction->code,
            "document_id" => $user_document,
            "document_type" => "CNPJ",
            "fee_bear" => "merchant",
            "method" => "PIX",   // fixed value => PIX
            "notify_url" => "https://hook.fastpayments.com.br/api/luxtakwithdrawhook",
            "account" => $new_pixkey, // chave pix
            "account_type" => $type_pixkey,  // tipo de chave pix
            "source_currency" => "BRL",
            "additional_remark" => "Pagamento numero ".$transaction->code,
        ...
        CURLOPT_HTTPHEADER => array(
            'AppId: 60D39793C7A44A33B9A0BDA528284821',
		...
		if (curl_errno($curl)) {
            ...
            if(isset($get_response['data']['id'])){
                ...
                try{
                    ...
                    //Do update
                    $transaction->update([
                        "payment_id" => $get_response['data']['id']
                    ]);
